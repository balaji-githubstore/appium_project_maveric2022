...
public class Demo4ActionScroll2NativeApp2 {
	...
	public static void main(String[] args) throws MalformedURLException, InterruptedException {
		...
		// presence
		int size = driver.findElements(AppiumBy.iOSNsPredicateString("name == 'test-CHECKOUT'")).size();
		...
		{		
			action.tick(finger.createPointerMove(Duration.ofMillis(0), PointerInput.Origin.viewport(), 364, 503))
			.tick(finger.createPointerDown(PointerInput.MouseButton.MIDDLE.asArg()))
			.tick(new Pause(finger, Duration.ofMillis(600)))
			.tick(finger.createPointerMove(Duration.ofMillis(600), PointerInput.Origin.viewport(), 363, 414))
